...
4. **Four-dropdown entry UX** (user-specified): shape (with small top-view thumbnail) → number of cabinets → cabinet module size → appliance sizes; every change regenerates the preview live; conflicts constrain the dropdown or flag — never silently grow the room. **Placement (user directive 2026-07-27): the dropdowns live in the configurator's TOP FRAME, adjacent to the room-dimension inputs** — inline and always visible, not a separate modal; the transient picker remains only as the backend/menu entry path.
