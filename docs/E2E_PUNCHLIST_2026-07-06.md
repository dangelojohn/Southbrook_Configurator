...
|---|------|-----------|--------|
| C1 | **CONFIRMED (code) — and the prior fix targeted the WRONG cause.** `sale.order.action_config_start` (`sale_order.py:220`) calls `get_wizard_action()` with no pre-created wizard, so the form opens as a virtual record where the delegated `product_tmpl_id` selector can't be committed → greyed. The July-4 "fix" set `allow_preset_selection=False` (blaming a confusing empty Preset field), and its tests only assert the returned **action dict**, never render the view — which is why it recurred. **COMPLICATION found while reading code:** `product.configurator _inherits product.config.session`, whose `product_tmpl_id` is NOT NULL, and Southbrook's `create()` override (`product_configurator.py:43-50`) hard-raises the exact error if a wizard is created without a template — so the agent's "pre-create a template-less wizard" fix can't work as-is. **Two competing theories (author's vs agent's) → must reproduce live before fixing.** | CONFIRMED cause, fix needs live repro | OPEN |
...
| L3 | **CONFIRMED — and worse than "polish".** Not a cosmetic string: Hermes **demo mode** returns a synthetic `overall_confidence=0.9`, which defeats the wizard's 0.85 "don't overwrite real data" guard (`hermes_wizard.py:180,660-702`), so clicking Apply **permanently overwrites `product.template.description_sale`** with the mock text (`hermes_service.py:239-242`). Odoo core then copies that onto `name` of every future `sale.order.line` for that product → it lands on customer quotes/PDFs. **Fix (code):** force demo-mode confidence below threshold OR skip `description_sale` in demo mode OR block auto-apply in demo. **Plus data cleanup:** scrub the string from polluted `description_sale` + existing order-line names. Reclassify as HIGH (customer-facing + writes to product master). | CONFIRMED (reclassify HIGH) | OPEN |
