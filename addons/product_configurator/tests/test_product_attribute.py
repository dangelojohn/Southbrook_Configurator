...
class ProductAttributes(BaseCommon):
    ...
    def test_15_name_search_with_invalid_template(self):
        # 19.0: env.context is read-only -> use with_context to attach
        # the configurator's tmpl-id key for this call. name_search's
        # `args=` was also renamed to `domain=`.
        result = self.env["product.attribute.value"].with_context(
            _cfg_product_tmpl_id=999999
    ...
    def test_16_name_search_with_no_attributes(self):
        ...
        result = self.env["product.attribute.value"].with_context(
            _cfg_product_tmpl_id=empty_product_template.id
        ).name_search(name="Red", domain=[], operator="ilike", limit=10)
        self.assertEqual(
            result,
            [],
