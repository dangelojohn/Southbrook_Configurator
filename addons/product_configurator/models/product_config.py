...
class ProductConfigSession(models.Model):
    ...
    # [REF] (f) — saved-configurations feature
    # ----------------------------------------
    # A buyer who configures a high-ticket product (luxury vehicle,
    # specialty equipment) often wants to save their configuration and
    # return to it later — possibly across browser sessions or devices.
    # Without this, the only way to "save" is to add to cart, which
    # commits to a partial purchase. These fields + the action below
    # enable a true bookmark flow: name a configuration, return to it
    # from the portal "My configurations" page.
    #
    # Field-naming note: ``name`` is already used on this model as the
    # auto-generated "Configuration Session Number" (line ~506). We
    # use ``bookmark_name`` here to avoid stomping that — the OCA
    # ``name`` is system-assigned; this one is buyer-supplied.
    bookmark_name = fields.Char(
        string="Bookmark Name",
        help="Optional buyer-supplied name for a saved configuration "
        "(e.g. 'Father's Day gift', 'Track car spec'). Empty for "
        "draft sessions that have not been saved.",
    )
    is_saved = fields.Boolean(
        string="Saved",
        default=False,
        help="True when the buyer has explicitly saved this "
        "configuration via the 'Save Configuration' action. Saved "
        "sessions are preserved by the GC cron regardless of age — "
        "they're an active bookmark, not abandoned state.",
    )

    def action_save_config(self, name=None):
        """Mark this session as a saved bookmark with an optional name.

        :param str name: optional buyer-supplied label. Blank/None
            leaves the existing bookmark_name alone (or empty).
        :return: True
        """
        self.ensure_one()
        vals = {"is_saved": True}
        if name:
            # Trim and bound the name to avoid storefront XSS via
            # title-bar injection or DB bloat from runaway values.
            vals["bookmark_name"] = name.strip()[:128]
        self.write(vals)
        return True

    def get_config_stock_info(self):
        """Compute aggregate stock availability for the current
        configuration's selected attribute values.

        For each attribute value selected on this session, look up
        the option-product (``product.attribute.value.product_id``).
        Aggregate min-quantity across all option-products — the
        configuration is constrained by the LEAST-AVAILABLE
        component. A buyer who configures a combination needs every
        option-product to be available; a single out-of-stock part
        makes the whole configuration out-of-stock.

        Returns a dict suitable for surfacing in the wizard UI:

          status         — one of:
            'no_components' (selected values have no option-products)
            'in_stock'      (all option-products qty_available > threshold)
            'low_stock'     (any option-product 0 < qty_available <= threshold)
            'out_of_stock'  (any option-product qty_available <= 0)
          min_qty_available     — min over option-products' on-hand qty
          min_virtual_available — min over option-products' forecasted qty
          components_checked    — count of option-products inspected
          components_constraint — the option-product that's the bottleneck
                                  (the one with the lowest qty_available)

        Threshold is read from the ir.config_parameter
        ``product_configurator.low_stock_threshold`` (default 5).
        Use sudo() because public-anonymous buyers don't have ACL
        read on option-product stock fields by default.
        """
        self.ensure_one()
        threshold_param = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("product_configurator.low_stock_threshold", default="5")
        )
        try:
        ...
        # Find all option-products attached to currently-selected
        # attribute values. Sudo because public buyers lack ACL on
        # the auxiliary option-product rows (see PR #2's
        # admin-tour-blind-spot disclosure for the same pattern).
        option_products = (
            self.sudo()
            .value_ids.mapped("product_id")
            .filtered(lambda p: p)
        )
        ...
        if not option_products:
            return {
                ...
                "min_qty_available": 0.0,
                "min_virtual_available": 0.0,
                "components_checked": 0,
                "components_constraint": False,
                "threshold": threshold,
            }

        # Aggregate. Track the constraint (min on-hand) for UX hint.
        constraint = option_products.sorted(lambda p: p.qty_available)[:1]
        min_qty = min(option_products.mapped("qty_available"))
        min_virtual = min(option_products.mapped("virtual_available"))

        if min_qty <= 0:
            status = "out_of_stock"
        elif min_qty <= threshold:
            status = "low_stock"
        else:
            status = "in_stock"
        ...
        return {
            "status": status,
            "min_qty_available": min_qty,
            "min_virtual_available": min_virtual,
            "components_checked": len(option_products),
            "components_constraint": {
                "id": constraint.id,
                "name": constraint.display_name,
                "qty_available": constraint.qty_available,
                "virtual_available": constraint.virtual_available,
            },
            "threshold": threshold,
        }
    ...
    @api.model
    def _gc_draft_sessions(self, days=None):
        """Garbage-collect stale draft configuration sessions.

        Every storefront wizard visit (anonymous OR authenticated)
        creates a ``product.config.session`` row. Without periodic
        cleanup, the table grows unbounded — a modest-traffic site
        accumulates 100k+ rows per month. ``done``-state sessions are
        retained (they reference a resolved ``product.product`` and
        are part of the audit trail for any cart line that uses them);
        only stale ``draft`` sessions are eligible for collection.

        :param int days: retention threshold in days. Defaults to the
            value of the ``product_configurator.session_gc_days``
            ``ir.config_parameter`` (which itself defaults to 7).
        :return: int — number of sessions deleted.
        """
        if days is None:
            param = self.env["ir.config_parameter"].sudo().get_param(
                "product_configurator.session_gc_days", default="7"
            )
            try:
                days = int(param)
            except (TypeError, ValueError):
                days = 7
        if days < 1:
            # Refuse to GC anything younger than a day — guards against
            # a misconfigured parameter wiping live sessions.
            _logger.warning(
                "product.config.session GC skipped: configured days=%s < 1",
                days,
            )
            return 0
        cutoff = fields.Datetime.now() - timedelta(days=days)
        # Use sudo: cron runs as the system user (uid=1) which has
        # broad ACL, but be explicit about the privilege requirement.
        # Exclude sessions referenced by any sale.order.line — even
        # draft sessions may be linked from a cart line via the
        # config_session_id FK; deleting one would orphan the cart.
        ...
        # The ``config_session_id`` field on ``sale.order.line`` is
        # contributed by the ``product_configurator_sale`` module, not
        # by this one. When ``_sale`` isn't installed, there are no
        # cart lines to protect; skip the lookup. Without this guard,
        # the GC method raises KeyError on bases that have the
        # configurator engine but not the sale integration.
        if "sale.order.line" in self.env.registry.models:
            referenced_ids = set(
                self.env["sale.order.line"]
                .sudo()
                .search([("config_session_id", "in", stale.ids)])
                .mapped("config_session_id")
                .ids
            )
        else:
            referenced_ids = set()
        to_unlink = stale.filtered(lambda s: s.id not in referenced_ids)
        count = len(to_unlink)
        if count:
            to_unlink.unlink()
            _logger.info(
                "product.config.session GC: deleted %d stale draft session(s) "
                "older than %d days (skipped %d referenced by cart lines)",
                count,
                days,
                len(stale) - count,
            )
