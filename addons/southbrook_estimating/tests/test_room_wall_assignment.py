...
class TestRoomWallAssignment(TransactionCase):
    ...
    def _create_line(self, **extra):
        # v19 onchange dispatch does NOT auto-populate `name` on raw .create()
        # (only Form does); sale.order.line.name is NOT NULL so every create
        # must pass it explicitly. Default to the product's display name.
        vals = {
