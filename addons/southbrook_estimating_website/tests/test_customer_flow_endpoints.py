...
class TestCustomerFlowEndpoints(TransactionCase):
    ...
    def test_url_mode_is_case_sensitive_for_internal_user(self):
        """?mode=Customer (capitalised) must NOT force customer mode for an
        internal user — exact parity with the old case-sensitive client
        check `=== "customer"`, so a dealer isn't dropped into the reduced
        customer surface by a stray capital / trailing space."""
        controller = ctrl_main.SouthbrookOrderBuilderPortal()
        controller._prepare_portal_layout_values = lambda: {}
        for variant in ("Customer", "CUSTOMER", "customer "):
            with stubbed_request(self.env, args={"mode": variant}):
                values = controller._prepare_southbrook_portal_values(self.order)
            self.assertEqual(
                values["order_mode"], "dealer",
                f"?mode={variant!r} must stay dealer for an internal user",
            )
